...
| `NG_ENTITY_QUERY_THRESHOLD` | `0.75` | косинус ≥ этого сопоставляет `object` запроса applicable с канонической сущностью (Giga: синонимы 0.77–0.87, несвязанные объекты ≤ 0.68) |
...
| `NG_RESTRICTION_REEMBED_ON_STARTUP` | `true` | пересчитать векторы ограничений, построенные по старому тексту эмбеддинга (только эмбеддинги, без LLM) |
