...
PROMPT_DESCRIPTION = (
    ...
    "Не извлекай ограничений, которых нет в тексте. Номер раздела или пункта не является "
    "значением ограничения: заголовок «3 Этажность жилых домов» не задает максимум 3 этажа. "
    "Для расстояния от A до B subject=A, object=B. Для «для каждого B должна быть хотя бы "
    "одна A» subject=A, object=B: проверяется каждый B, а не каждый A. Не извлекай "
    "«одна школа» как отдельную норму, если это часть наличия школы в радиусе. "
    "Для этажности subject=территория применимости, object=здание; этажность — показатель. "
    "«Контуры жилых домов» — геометрия объекта «жилой дом», не отдельный тип объекта."
)
